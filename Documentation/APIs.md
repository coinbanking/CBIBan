...
To use `CBIban`, we can drag and drop a UIKit element and connect it with `Animatable` UI element in Identity inspector. Here are the supported `Animatable` UI elements to map UIKit elements.
...
`CBIban` provides a set of Designable protocols as below. Because of the power of protocol-oriented programming in Swift, we don't even have to use Animatable default UI elements e.g. `AnimatableView` to unlocked the power of `CBIban`. We can conform to `CBIban` protocols to use the default implementation in protocol extension to create other custom UI elements.
...
| Property name | Data type | Description |
| ------------- |:-------------:| ----- |
| borderType | Optional&lt;String> | border type: `solid`, `dash`, also can be found in emum [`BorderSide`](../CBIban/BorderType.swift). If not specified, then display solid borders. **Please notice**: If we use `maskType` property then `borderType` will be ignored. |
...
| borderSide | Optional&lt;String> | border side: `top`, `right`, `bottom` or `left`, also can be found in emum [`BorderSide`](../CBIban/BorderSide.swift). Multiple sides can be configured with a comma separated list of sides (e.g. `top,bottom`). If not specified, then display four sides. **Please notice**: If we use `maskType` property then `borderSide` will be ignored. |
...
| predefinedColor | Optional&lt;String> | Predefined color. All predefined colors are in enum [`ColorType`](../CBIban/ColorType.swift). To find the predefined flat colors, you can use [flatuicolors.com](https://flatuicolors.com), all flat color start with `Flat`, e.g. `PETER RIVER` on the website maps to `FlatPeterRiver` in the enum. |
...
| predefinedGradient | Optional&lt;String> | Predefined gradients. All predefined gradients are in enum [`GradientType`](../CBIban/GradientType.swift), To find the predefined gradients, you can use [uigradients.com](http://uigradients.com), e.g. `Juicy Orange` on the website maps to `JuicyOrange` in the enum. |
| startPoint | Optional&lt;String> | start direction point, can find in enum [`GradientStartPoint `](../CBIban/GradientStartPoint.swift). |
...
| Property name | Data type | Description |
| ------------- |:-------------:| ----- |
| maskType | Optional&lt;String> | Supported maks type: All predefined animations are in enum [`MaskType`](../CBIban/MaskType.swift) |
...
| Property name | Data type | Description |
| ------------- |:-------------:| ----- |
| animationType | Optional&lt;String> | Supported animations. All predefined animations are in enum [`AnimationType`](../CBIban/AnimationType.swift) |
...
| transitionDuration | Double | transition duration. The default value is defined in [`Constants`](../CBIban/Constants.swift) (0.7 seconds) |
| interactiveGestureType | Optional&lt;String> | interactive gesture type. used to specify the gesture to dismiss/pop current scence. All supported interactive gesture types are in [`InteractiveGestureType`](../CBIban/InteractiveGestureType.swift) |
